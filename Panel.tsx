...
interface PanelProps {
  ...
  onDownload: () => void;
  onReset: () => void;
  onRegenerate?: (beatNum: number, caption: string, dialogue: string) => void;
}
...
export const Panel: React.FC<PanelProps> = ({ face, allFaces, onChoice, onOpenBook, onDownload, onReset, onRegenerate }) => {
  ...
  // Cover "read" gate: ready once the first story beats have art.
  const storyFaces = allFaces.filter((f) => f.type === "story").sort((a, b) => (a.beatNum || 0) - (b.beatNum || 0));
  const readyStories = storyFaces.filter((f) => f.imageUrl).length;
  const gateReady = readyStories >= 1;
  ...
  const handleEditSave = (e: React.MouseEvent) => {
    e.stopPropagation();
    setIsEditing(false);
    if (onRegenerate && face.beatNum !== undefined) onRegenerate(face.beatNum, editCaption, editDialogue);
  };
    ...
    <div className={`panel-container relative group ${isFullBleed ? "!p-0 !bg-[#0a0a0a]" : ""} ${face.isLoading ? "opacity-70 grayscale-[50%]" : ""}`}>
      ...
      {isEditing && (
        <div onClick={(e) => e.stopPropagation()} className="absolute inset-4 bg-white/95 border-4 border-black p-4 z-40 flex flex-col font-sans">
          ...
          <div className="flex-1 overflow-y-auto space-y-2">
            ...
            <p className="text-xs text-red-600 font-bold">* Saving will redraw the artwork with the new text.</p>
          </div>
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button onClick={handleEditSave} className="px-4 py-2 border-2 border-black bg-green-400 font-comic uppercase">Redraw</button>
          </div>
        </div>
      )}
      ...
      {face.isLoading && face.imageUrl && (
        <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-30">
          <div className="bg-white p-4 border-4 border-black rotate-[-2deg] font-comic font-bold text-xl animate-pulse">REDRAWING...</div>
        </div>
      )}
      ...
      {/* Error fallback */}
      {face.error && !face.imageUrl && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 text-center p-6">
          <p className="font-comic text-2xl text-red-600 mb-2">Panel hiccup!</p>
          <p className="text-sm text-gray-600">This page couldn't be drawn. Check your API key/connection — other pages keep going.</p>
        </div>
      )}

      {/* Recap "Previously..." overlay */}
      {face.type === "recap" && (
        <div className="absolute inset-0 z-20 flex flex-col justify-end p-5 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
          <div className="bg-yellow-300 border-2 border-black px-2 py-0.5 w-fit font-comic uppercase text-sm mb-2 -rotate-2">Previously...</div>
          <p className="text-white font-sans text-sm md:text-base leading-snug" style={{ textShadow: "1px 1px 2px black" }}>{face.recapText}</p>
        </div>
      )}
      ...
      {face.isDecisionPage && face.choices.length > 0 && (
        <div className={`absolute bottom-0 inset-x-0 p-6 pb-12 flex flex-col gap-3 items-center justify-end transition-opacity duration-500 ${face.resolvedChoice ? "opacity-0 pointer-events-none" : "opacity-100"} bg-gradient-to-t from-black/90 via-black/50 to-transparent z-20`}>
          <p className="text-white font-comic text-2xl uppercase tracking-widest animate-pulse">What do you do?</p>
          {face.choices.map((choice, i) => (
            <button key={i} onClick={(e) => { e.stopPropagation(); if (face.beatNum) onChoice(face.beatNum, choice); }}
      ...
      {face.type === "back_cover" && (
        <div className="absolute bottom-24 inset-x-0 flex flex-col items-center gap-4 z-20">
          ...
          <button onClick={(e) => { e.stopPropagation(); onReset(); }} className="comic-btn bg-green-500 text-white px-8 py-4 text-2xl font-bold hover:scale-105">Next Issue →</button>
        </div>
      )}
    </div>
  );
